...
export default function ResetPassword() {
  ...
  const inputClass = `w-full rounded-xl border px-4 py-3 text-sm font-medium text-slate-800 placeholder-slate-400 transition-all duration-200 bg-white/60 focus:outline-none focus:ring-2 border-white/40 focus:ring-[#d66847]/40 focus:border-[#d66847]`;
      ...
      >
        ...
        >
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
            {success && (
              <div className="rounded-xl border border-green-400/40 bg-green-500/10 px-4 py-3 text-sm font-medium text-green-300 text-center">
                {success}
                <br/>
                <Link to="/login" className="font-bold underline mt-2 inline-block text-white hover:text-[#d66847] transition">Go to Login</Link>
              </div>
            )}
            ...
            {!success && (
              <>
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="group relative w-full overflow-hidden rounded-[14px] px-6 py-3.5 text-sm font-bold uppercase tracking-widest text-white transition-all duration-300 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed mt-2"
                  style={{
